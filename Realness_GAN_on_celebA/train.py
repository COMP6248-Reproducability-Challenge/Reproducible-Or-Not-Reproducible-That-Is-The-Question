...
if __name__ == '__main__':                             # dataloader uses multithreading outside of python GIL, therefore have to have main defined if running on windows
  #  torch.multiprocessing.freeze_support()             # may also need to use this
    ...
    if param.load_ckpt is None: 
        temp = base_dir + "/images"
        print(temp)
        os.system('mkdir -p %s' % base_dir)
        if not os.path.exists(temp):
            os.makedirs(temp)
        if param.gen_extra_images > 0 and not os.path.exists(param.extra_folder):
            os.mkdir(param.extra_folder)
        print_now(param)
    ...
    # for root, dirs, files in os.walk(param.input_folder):
    #     for filename in files:
    #         print(filename)

    print(param.input_folder)
    ...
    for i in range(iter_offset, param.total_iters):
        ...
        if i==0 or (i+1) % 100 == 0: #update SWD
            imgs_real, _ = random_sample.__next__()
            imgs_fake = G(z_test)
           # print(imgs_real.shape,imgs_fake.shape,imgs_real.device,imgs_fake.device)
            swd_score = swd(imgs_real.to(device), imgs_fake.to(device), device=device)


        # print progress
        if i < 1000 or (i+1) % 100 == 0:
            end = time.time()
            fmt = '[%d / %d] SD: %d Diff: %.4f loss_D: %.4f loss_G: %.4f time:%.2f   SWD: %.2f'
